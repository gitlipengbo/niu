...
<title><?php echo ($room["roomid"]); ?>号十二人斗牌房间</title>

<link rel="stylesheet" type="text/css" href="http://cdn.euo69.cn/app/css/bull_vue-1.0.4.css?1519886810810">
<link rel="stylesheet" type="text/css" href="http://cdn.euo69.cn/app/css/alert-1.1.css?1519886810810">
...
<script src="/app/js/game10.js" type="text/javascript"></script> 
...
function randomString(len) {
  ...
  var title  = '<?php echo ($room["roomid"]); ?>号十二人斗牌房间';//分享的标题
  var shareimg = 'http://cdn.euo69.cn/themes/img/activity/test/10.png';//分享的图片
 var desc = '模式： <?php echo ($room['wfname']); if($room['df']): ?>底分：<?php echo ($room['df']); endif; if($room['gz']): ?>规则： <?php echo ($room['gz']); endif; ?> <?php if($room['px']): ?>牌型： <?php if(is_array($room['px'])): foreach($room['px'] as $key=>$one): echo ($one); endforeach; endif; endif; ?> <?php if($room['sz']): ?>上庄： <?php echo ($room['sz']); endif; ?> <?php if($room['zjs']): ?>局数：<?php echo ($room['zjs']); endif; ?>';//分享的描述信息
...
<textarea id="room_str" value="">牛来了 12人斗牌 房间号：<?php echo ($room["roomid"]); ?>'模式<?php echo ($room['wfname']); if($room['df']): ?>底分：<?php echo ($room['df']); endif; if($room['gz']): ?>规则： <?php echo ($room['gz']); endif; ?> <?php if($room['px']): ?>牌型： <?php if(is_array($room['px'])): foreach($room['px'] as $key=>$one): echo ($one); endforeach; endif; endif; ?> <?php if($room['sz']): ?>上庄<?php echo ($room['sz']); endif; ?> ' 点击加入http://<?php echo ($_SERVER['HTTP_HOST']); ?>/portal/index/room.html?room=<?php echo ($room["id"]); ?>#num55688
</textarea>
...
<img src="http://cdn.euo69.cn/app/img/bull9/btn_game_rule.png" class="bGameRule jiurenniuniu-bGameRule" onclick="opnemm('fangjian_gz','vroomRule')">  
   <img src="/app/img/bull9/btn_game_home.png" class="return"  onclick="opnemm('fangjian_fanhuisy','tishi')" /> 
   <img src="/app/img/bull9/btn_game_rule.png" class="bGameRule shier-bGameRule" onclick="opnemm('fangjian_gz','vroomRule')" /> 
...
">
   ...
   <div class="bottomMessage"  onclick="opnemm('fangjian_kj','message')">
    <img src="/app/img/bull9/bull_message.png" class='shier-bottomMessage-img'  />
   </div> 
...
"></div> 
   ...
   <div id="table" class="table">
    <img src="/app/img/bull12/bull_bg.png" class="tableBack" /> 
