...
<title><?php echo ($room["roomid"]); ?>号十人斗牌房间</title>
...
<script src="/app/js/game2.js" type="text/javascript"></script> 
...
function randomString(len) {
  ...
  var title  = '<?php echo ($room["roomid"]); ?>号十人斗牌房间';//分享的标题
  var shareimg = 'http://cdn.euo69.cn/themes/img/activity/test/13.png';//分享的图片
...
<textarea id="room_str" value="">牛来了 10人斗牌 房间号：<?php echo ($room["roomid"]); ?>'模式<?php echo ($room['wfname']); if($room['df']): ?>底分：<?php echo ($room['df']); endif; if($room['gz']): ?>规则： <?php echo ($room['gz']); endif; ?> <?php if($room['px']): ?>牌型： <?php if(is_array($room['px'])): foreach($room['px'] as $key=>$one): echo ($one); endforeach; endif; endif; ?> <?php if($room['sz']): ?>上庄<?php echo ($room['sz']); endif; ?> ' 点击加入http://<?php echo ($_SERVER['HTTP_HOST']); ?>/portal/index/room.html?room=<?php echo ($room["id"]); ?>#num55688
</textarea>
