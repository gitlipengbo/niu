<?php if (!defined('THINK_PATH')) exit();?><html>
 <head> 
  ...
  <title><?php echo ($room["roomid"]); ?>号六人三公房间</title> 
  <link rel="stylesheet" type="text/css" href="/app/css/bullshop.css" /> 
  <link rel="stylesheet" type="text/css" href="http://cdn.euo69.cn/app/css/sangong-1.0.0.css?1519886810810">
...
function randomString(len) {
  ...
  var title  = '<?php echo ($room["roomid"]); ?>号六人三公房间';//分享的标题
  var shareimg = 'http://cdn.euo69.cn/themes/img/activity/test/3.png';//分享的图片
...
<?php if(count($room['userlist']) >= 6 && $room['userlist'][$user['id']] != 1): ?><div id="valert2" class="alert">
  ...
  <?php exit(); endif; ?>
   ...
   <img src="/app/img/sangong/home.png" class="return" style="right: 10px;" onclick="opnemm('fangjian_fanhuisy','tishi')" /> 
<textarea id="room_str" value="">牛来了 6人三公 房间号：<?php echo ($room["roomid"]); ?>'模式<?php echo ($room['wfname']); if($room['df']): ?>底分：<?php echo ($room['df']); endif; if($room['gz2']): ?>规则： <?php if(is_array($room['gz2'])): foreach($room['gz2'] as $key=>$one): echo ($one); endforeach; endif; endif; ?> ' 点击加入http://<?php echo ($_SERVER['HTTP_HOST']); ?>/portal/index/room.html?room=<?php echo ($room["id"]); ?>#num55688
</textarea>
...
<img src="http://cdn.euo69.cn/app/img/public/share.png" class="copyUrl" data-clipboard-action="copy" data-clipboard-target="#copyUrl"/> 
   ...
   <img src="/app/img/sangong/icon_game_rule.png" class="bottomGameRule" style="z-index: 110;" onclick="opnemm('fangjian_gz','vroomRule')"/> 
   <img src="/app/img/sangong/icon_sound61.png" class="bottomGameIntro" style="z-index: 110;" 
...
<?php if((count($room['userlist']) > 0 && $room['js'] == 0) || ($room['js'] > 0 && !$room['userlist'][$user['id']])): ?><div id="valert" class="alert">
   ...
   <div class="mainPart" style="margin-top: -34vh;">
    ...
    <?php $count=M('user_room')->where(array('uid'=>$user['id']))->count(); $count=$count+1; $max=M('user_room')->where(array('uid'=>$user['id']))->order('jifen desc')->find(); ?> 
