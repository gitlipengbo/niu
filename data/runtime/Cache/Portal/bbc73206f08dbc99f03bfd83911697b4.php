...
<title><?php echo ($room["roomid"]); ?>号九人飘三叶房间</title>
<link rel="stylesheet" type="text/css" href="/app/css/flower-9-2.0.3.css" />
...
.copyUrl{  
  ...
  <script src="/app/js/app.js?v=<?php echo time(); ?>" type="text/javascript"></script>
  <script src="/app/js/game16.js?v=<?php echo time(); ?>" type="text/javascript"></script>
...
function randomString(len) {
  ...
  var title  = '<?php echo ($room["roomid"]); ?>号九人飘三叶房间';//分享的标题
  var shareimg = 'http://'+window.location.hostname+'/app/9.png';//分享的图片
  var desc = '<?php if($room['gz2']): ?>牌型： <?php if(is_array($room['gz2'])): foreach($room['gz2'] as $key=>$one): echo ($one); endforeach; endif; endif; ?> <?php if($room['cm']): ?>筹码:<?php echo ($room['cm']); endif; if($room['sx']): ?>上限:<?php echo ($room['sx']); endif; ?> <?php if($room['zjs']): ?>局数：<?php echo ($room['zjs']); endif; ?>';//分享的描述信息
...
<!--
 ...
 --> 
   <img class="kadun kadunTop" src="http://cdn.euo69.cn/app/img/kadun1.png" onclick="location.reload()">
  ...
  <img src="/app/img/footer_bull.png" usemap="#planetmap" class="bottom" /> 
  <img src="/app/img/icon_game_rule.png" class="bottomGameRule" style="z-index: 110;" onclick="opnemm('fangjian_gz','vroomRule')"/> 
<textarea id="room_str" value="">牛来了 9人飘三叶 房间号:<?php echo ($room["roomid"]); ?>'<?php if($room['gz2']): ?>牌型： <?php if(is_array($room['gz2'])): foreach($room['gz2'] as $key=>$one): echo ($one); endforeach; endif; endif; ?> <?php if($room['cm']): ?>筹码:<?php echo ($room['cm']); endif; if($room['sx']): ?>上限:<?php echo ($room['sx']); endif; ?> '; 点击加入http://<?php echo ($_SERVER['HTTP_HOST']); ?>/portal/index/room.html?room=<?php echo ($room["id"]); ?>#num55688
</textarea>
...
<div class="playerPK" style="display: none;">
  ...
  <div id="table" class="table" style="width: 100%; height: 100%;">
   <img src="/app/img/table-zhajinhua.png" class="tableBack" />
